...
from .hypothesis_tests import SequentialTestParametricBootstrap
...
def get_tests(*, seed: Seed, target_model, N_bootstrap, N_sequences, desired_level, alphabet_is_ordered, markov_order,
    ...
    stein_ops = {
        ...
        r"$\mathrm{ZS}_1$": dict(
            ...
            op=ZanellaSteinOperator(
                ...
                action_set=(
                        ...
                        | InsertActionSet(complement=True, min_ix=0, max_ix=0, xs=target_model.alphabet)
                        | DelActionSet(complement=True, min_ix=0, max_ix=0)
                ),
                kappa='barker',
            ),
            is_stein=True,
        ...
        r"$\mathrm{ZS}_1'$": dict(
            ...
            op=ZanellaSteinOperator(
                model=target_model,
                action_set=SubActionSet(complement=True, min_ix=0, max_ix=0, xs=target_model.alphabet),
                kappa='barker',
            ),
            is_stein=True,
        ),
        r"$\mathrm{ZS}_3$": dict(
            ...
            op=ZanellaSteinOperator(
                ...
                action_set=(
                        ...
                        | InsertActionSet(complement=True, min_ix=0, max_ix=2, xs=target_model.alphabet)
                        | DelActionSet(complement=True, min_ix=0, max_ix=2)
                ),
                kappa='barker',
            ),
            is_stein=True,
        ...
        stein_ops |= {
            r"$\mathrm{ZS}_" + f"{n}$": dict(
                ...
                op=ZanellaSteinOperator(
                    model=target_model,
                    action_set=(
                            SubActionSet(complement=True, min_ix=0, max_ix=n - 1, xs=target_model.alphabet)
                            | InsertActionSet(complement=True, min_ix=0, max_ix=n - 1, xs=target_model.alphabet)
                            | DelActionSet(complement=True, min_ix=0, max_ix=n - 1)
                    ),
                    kappa='barker',
                ),
                is_stein=True,
            ...
            r"$\mathrm{ZS}_" + f"{n}'$": dict(
                ...
                op=ZanellaSteinOperator(
                    model=target_model,
                    action_set=SubActionSet(complement=True, min_ix=0, max_ix=n - 1, xs=target_model.alphabet),
                    kappa='barker',
                ),
                is_stein=True,
            ),
    ...
    def make_sequential_stein_test(rng, kernel, desired_level, target_model, **kwargs):
        kwargs['n_bootstrap'] = int(np.ceil(kwargs['n_bootstrap'] * 1.1))
        action_sets_and_weights = [
            (SubActionSet(complement=True, min_ix=0, max_ix=2, xs=target_model.alphabet), 1 / 3),
            (SubActionSet(complement=True, min_ix=3, max_ix=6, xs=target_model.alphabet), 1 / 3),
            (SubActionSet(complement=True, min_ix=7, max_ix=8, xs=target_model.alphabet), 1 / 3),
        ]
        ...
        return SequentialTestParametricBootstrap(
            tests_and_weights=tests_and_weights,
            desired_level=desired_level,
            rng=rng,
            target_model=target_model,
    ...
    if alphabet_is_ordered:
        ...
        better_kernel = 'CSK', ContiguousSubsequenceKernel(min_length=markov_order + 1, max_length=markov_order + 1)
    best_kernel = 'String', StringKernel(substring_length=None, lmbda=0.1)

    for kernel, n_mmd_samples in [(best_kernel, 100)]:
        tests |= {
        ...
        tests |= {
            ...
            for k, v in {
                ...
                r"$\mathrm{ZS}_9'$": dict(
                    ...
                    op=ZanellaSteinOperator(
                        model=target_model,
                        action_set=SubActionSet(complement=True, min_ix=0, max_ix=8, xs=target_model.alphabet),
                        kappa='barker',
                    ),
                    is_stein=True,
                ),
    ...
    for kernel, is_expensive in kernels:
        ...
        tests |= {
            ...
            f'Seq-Stein(k={kernel[0]})': dict(
                version='4',
                test=hold(make_sequential_stein_test)(
                    kernel=kernel[1],
                    target_model=target_model,
                    **test_kwargs
                ),
    ...
    def go():
        ...
        for scenario_name, scenario in scenarios.items():
            ...
            for test_name, test in tests.items():
                ...
                scenario_part_unsafe = f"{slugify_unsafe(scenario_name)}-{scenario['version']}"
                test_part_unsafe = f"{slugify_unsafe(test_name)}-{test['version']}"
                ...
                cache_name = f'../figures/suite/{suite_part}__{scenario_part_unsafe}__{test_part_unsafe}.pickle'  # to store entire results
...
def display_groups():
    no_rename = lambda l: {s: s for s in l}
    all_scenarios = no_rename([
        ...
        'Binary sequences, sample is non-Markov',
    ])

    display_groups = {
        'MMD as a function of sample size (ExpDist)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r'LR($k=k_\mathrm{true}$)',
                r'LR($k=k_\mathrm{true}+1$)',
                r'LR($k=\infty$)',
                r"MMD(k=ExpDist, n=10)",
                r"MMD(k=ExpDist, n=33)",
                r"MMD(k=ExpDist, n=100)",
                r"MMD(k=ExpDist, n=333)",
                r"MMD(k=ExpDist, n=1000)",
            ]),
            x_axis_is_scenario=True,
        ),
        'MMD as a function of sample size (CSK)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r'LR($k=k_\mathrm{true}$)',
                r'LR($k=k_\mathrm{true}+1$)',
        ...
        'Stein, Gibbs vs. MMD as a function of neighbourhood (ExpDist)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"MMD(k=ExpDist, n=100)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mpf}'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{10,mpf}'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{\infty,mpf}'$)",
                r"Stein(k=ExpDist, op=Gibbs)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Stein, Gibbs vs. MMD as a function of neighbourhood (CSK)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"MMD(k=CSK, n=100)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,mpf}'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{10,mpf}'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{\infty,mpf}'$)",
                r"Stein(k=CSK, op=Gibbs)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Choice of balancing function (ExpDist)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mpf}'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mh}'$)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Choice of balancing function (CSK)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=CSK, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,mpf}'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,mh}'$)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Prefix-based Stein test': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,pre8}'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,pre8}'$)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Prefix-based MMD': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"MMD(k=ExpDist, n=100)",
                r"MMD(k=Pre(ExpDist,5), n=100)",
                r"MMD(k=CSK, n=100)",
                r"MMD(k=Pre(CSK,5), n=100)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Max-probability based test (ExpDist)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}''$)",
                r"Stein-MMD(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$, n=100)",
                r"MMD-Ap(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=1)",
                r"MMD-Ap(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=3)",
                r"MMD-Ap(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=5)",
                r"MMD-Ap(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=10)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Max-probability based test (CSK)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=CSK, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,mp5}''$)",
                r"Stein-MMD(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$, n=100)",
                r"MMD-Ap(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=1)",
                r"MMD-Ap(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=3)",
                r"MMD-Ap(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=5)",
                r"MMD-Ap(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$, n=100, T=10)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Min-probability based test (ExpDist)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,minp5}'$)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Min-probability based test (CSK)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=CSK, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,minp5}'$)",
            ]),
            x_axis_is_scenario=True,
        ),
        'Aggregated Stein test (ExpDist)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mp5}'$)",
                r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,pre8}'$)",
                r"Agg-Stein(k=ExpDist)",
                r'LR($k=k_\mathrm{true}$)',
                r'LR($k=k_\mathrm{true}+1$)',
                r'LR($k=\infty$)',
            ]),
        ...
        'Aggregated Stein test (CSK)': dict(
            scenarios=all_scenarios,
            tests=no_rename([
                r"Stein(k=CSK, op=$\mathrm{ZS}_3'$)",
                r"Stein(k=CSK, op=$\mathrm{ZS}_{3,mp5}'$)",
    ...
    display_groups = {
        ...
        'Few short sequences': dict(
            scenarios={
                'Random 2nd-order MC: Few short sequences': 'Random 2nd-order MC',
            },
            tests={
                s: s for s in [
                    r"MMD(k=ExpDist, n=100)",
                    # r"Agg-MMD(k=ExpDist, n=100)",
                    # r"Stein(k=ExpDist, op=$\mathrm{ZS}_3$)",
                    # r"Stein(k=ExpDist, op=$\mathrm{ZS}_3'$)",
                    r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mpf}'$)",
                    # r"Stein(k=ExpDist, op=$\mathrm{ZS}_{3,mh}'$)",
            ...
            x_axis_is_scenario=False,
        ),

        'Level check': dict(
            scenarios={
                'Random 2nd-order MC: Level check': 'Level check',
            },
            test_regexes_include=['.'],
            test_regexes_exclude=[],
            x_axis_is_scenario=False,
        ),
    }
...
def main():
    ...
    variables_to_display = [
        ...
        # ('avg_total_seconds', 'Average time to run test', 'timing'),
    ]
    variables_to_write = [
        'rejection_rate',
        'avg_total_seconds',
    ...
    if not plot:
        return
    for var_key, var_label, var_file_part in variables_to_display:
        to_display = {k: {k2: v2[var_key] for k2, v2 in v.items()} for k, v in results.items()}
        df = pd.DataFrame(to_display)
        print(df)
        df.index.name = 'Test'
        df.columns.name = 'Scenario'
        # df = df.T.sort_values(df.T.columns[0], ascending=False).T
        # df = df.sort_values(df.columns[2])
        series = df.unstack()
        series.name = var_label
        series = series.reset_index()

        # print(series)
        def plot(filtered_series, *, title=None, x, y, hue):
            if title is None:
                title = "All tests and scenarios"
            with pltctx('l'):
                chart = sns.barplot(data=filtered_series, x=x, y=y, hue=hue, palette='Set2')
                ...
                plt.title(title)
                plt.tight_layout()
                filename = f'../figures/suite-{var_file_part}-{slugify(title)}.pdf'
        ...
        y = var_label
        hue = 'Test'
        plot(series, title=None, x=x, y=y, hue=hue)
        for group_name, group in display_groups().items():
            filtered_df = df.filter(items=group['scenarios'].keys()).rename(columns=group['scenarios'])
            if 'tests' not in group:
                columns_to_keep = set()
                for regex in group['test_regexes_include']:
                    columns_to_keep |= set(filtered_df.T.filter(regex=regex).columns)
                for regex in group['test_regexes_exclude']:
                    columns_to_keep -= set(filtered_df.T.filter(regex=regex).columns)
                columns_to_keep = [c for c in df.index if c in columns_to_keep]
                filtered_df = filtered_df.T.filter(items=columns_to_keep).T
            else:
                filtered_df = filtered_df.T.filter(items=group['tests'].keys()).rename(columns=group['tests']).T
            filtered_series = filtered_df.unstack()
            filtered_series.name = var_label
            filtered_series = filtered_series.reset_index()
            x = 'Scenario'
            y = var_label
            hue = 'Test'
            if not group['x_axis_is_scenario']:
                x, hue = hue, x
            plot(filtered_series, title=group_name, y=y, x=x, hue=hue)
