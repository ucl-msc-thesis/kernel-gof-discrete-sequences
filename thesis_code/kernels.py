...
sys.path.append(str(Path(__file__).parent.parent / 'python_ssk'))
...
class StringKernel(Kernel):
    def __init__(self, substring_length: Optional[int], lmbda: float, accum: bool = True, normalize: bool = True):
    ...
    @property
    def params(self):
        return (self._substring_length, self._lmbda, self._accum)
    ...
    def compute_gram(self, X: collection_of_fv, Y: collection_of_fv) -> npt.NDArray:
        ...
        max_len = 0
        for k in range(N_p):
            for phi_p, weight_p in X[k].dict_items():
                phi_p = array.array('l', [x for x in phi_p])
                max_len = max(max_len, len(phi_p))
                X_concat.append(phi_p)
                weights = np.full(N_p, 0.)
                weights[k] = weight_p
                X_weights.append(weights)
        for l in range(N_q):
            for phi_q, weight_q in Y[l].dict_items():
                phi_q = array.array('l', [x for x in phi_q])
                max_len = max(max_len, len(phi_q))
                Y_concat.append(phi_q)
                weights = np.full(N_q, 0.)
                weights[l] = weight_q
                Y_weights.append(weights)
        X_concat_ = np.full((len(X_concat), 1), None, dtype='O')
        Y_concat_ = np.full((len(Y_concat), 1), None, dtype='O')
        X_concat_[:, 0] = X_concat
        Y_concat_[:, 0] = Y_concat
        X_concat = X_concat_
        Y_concat = Y_concat_
        X_weights = np.stack(X_weights)
        Y_weights = np.stack(Y_weights)
        substring_length = self._substring_length or max_len
        K_big = string_kernel(X_concat, Y_concat, substring_length, self._lmbda, accum=self._accum,
                              normalize=self._normalize)
        K = X_weights.T @ K_big @ Y_weights
        return K
