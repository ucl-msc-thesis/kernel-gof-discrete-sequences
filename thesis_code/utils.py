...
def client_with_code(address):
    client = Client(address)
    if os.path.exists('thesis_code'):
        prefix = '.'
    else:
        prefix = '..'
    subprocess.run(['rm', '-f', f'{prefix}/out/thesis_code.zip'])
    subprocess.run(['sh', '-c', f'cd {prefix}; zip -r out/thesis_code.zip thesis_code/*.py'])
    with open(f'{prefix}/out/thesis_code.zip', 'rb') as f:
        thesis_code_zip_data = f.read()
    ...
    client.register_worker_callbacks(
        setup=functools.partial(
            # we pretend it's a python egg, as zipimport fails (some bug related to zipimport caching?)
            _worker_upload, data=thesis_code_zip_data, fname='thesis_code.egg',
        )
    )
