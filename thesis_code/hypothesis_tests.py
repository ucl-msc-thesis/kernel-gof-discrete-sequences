...
class SequentialTestParametricBootstrap(HypothesisTest):
    def __init__(
            self, *,
            rng: np.random.Generator,
            target_model: SequenceDist,
            tests_and_weights: list[tuple[WithParametricBootstrap, float]],
            cumulative: bool,
            **kwargs,
    ):
        weights = [w for _, w in tests_and_weights]
        tests = [t for t, _ in tests_and_weights]
        sum_of_weights = sum(weights)
        assert np.isclose(sum_of_weights, 1), f"weights must sum to 1, sums to {sum_of_weights}"
        assert np.all(np.array(weights) > 0), f"weights must be positive, got {weights}"
        weights = np.cumsum(weights)
        n_bootstraps = [t._n_bootstrap for t, _ in tests_and_weights]
        self._n_bootstrap = n_bootstraps[0]
        assert np.allclose(n_bootstraps, self._n_bootstrap), "n_bootstrap must be the same for all tests"
        self._tests_and_weights = list(zip(tests, weights))
        self._bootstrap_sample_size = None
        self._sampling_timing = None
        self._target_model = target_model
        self._rng = rng
        self._cumulative = cumulative
        # todo - this does not do the "double bootstrap" and I think we highly likely need it
        super().__init__(**kwargs)
    ...
    def does_test_reject(self, samples: List[FreeVector]) -> TestResult:
        ...
        if self._bootstrap_sample_size is None or self._bootstrap_sample_size != sample_size:
            self._bootstrap_samples = np.full((self._n_bootstrap, len(self._tests_and_weights)), np.nan)
            self._bootstrap_sample_size = sample_size
            self._bootstrap_timing = np.full(len(self._tests_and_weights), datetime.timedelta(seconds=0))
            all_samples = deque()
            before = datetime.datetime.now()
            for i in range(self._n_bootstrap):
                samples = self._target_model.generate_samples(self._rng, sample_size)
            ...
            after = datetime.datetime.now()
            self._sampling_timing = after - before
            for j, (test, _) in enumerate(self._tests_and_weights):
                before = datetime.datetime.now()
                for i, samples in enumerate(all_samples):
                    self._bootstrap_samples[i, j] = test._compute_test_statistic(samples)
                after = datetime.datetime.now()
                self._bootstrap_timing[j] += after - before
            if self._cumulative:
                self._bootstrap_samples = np.cumsum(self._bootstrap_samples, axis=0)
        before = datetime.datetime.now()
        reject = False
        mask = np.full(self._n_bootstrap, False)
        bootstrap_timing_to_use = datetime.timedelta(seconds=0)
        for i, (test, weight) in enumerate(self._tests_and_weights):
            bootstrap_timing_to_use += self._bootstrap_timing[i]
            test_stat = test._compute_test_statistic(samples)
            critical_value = np.quantile(
                np.where(mask, np.inf, self._bootstrap_samples[:, i]),
                np.ceil(1 - weight * self._desired_level),
                interpolation='higher',
            )
            if critical_value < test_stat:
                reject = True
                break
            mask |= np.where(critical_value < self._bootstrap_samples[:, i], True, False)
        after = datetime.datetime.now()
        core_timing = after - before
        time_taken_to_eval = self._sampling_timing + bootstrap_timing_to_use + core_timing
        return TestResult(
            time_taken_to_eval=time_taken_to_eval,
            does_test_reject=reject,
        )
