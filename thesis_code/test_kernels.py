...
class TestStringKernel(unittest.TestCase):
    def test_expected(self):
        test_cases = lambda lmbda: [
            ('hallo', 'hello', 3, lmbda ** 6 + lmbda ** 8 + 4 * lmbda ** 10),
            ('car', 'cat', 2, lmbda ** 4),
            ('car', 'car', 2, 2 * lmbda ** 4 + lmbda ** 6),
            ('bar', 'cat', 2, 0),
            ('bar', 'cat', 1, lmbda ** 2),
        ]
        for lmbda in [0.2, 0.5, 1, 2]:
            for x, y, n, expected in test_cases(lmbda):
                with self.subTest(lmbda=lmbda, x=x, y=y, n=n):
                    x = [ord(c) for c in x]
                    y = [ord(c) for c in y]
                    k = StringKernel(substring_length=n, lmbda=lmbda, accum=False, normalize=False)
                    kxy = k.compute_gram(list_of_vec_of_single_seq(x), list_of_vec_of_single_seq(y)).item()
                    self.assertAlmostEqual(expected, kxy)

    def test_normalize(self):
        entropy = 315224592999591434235336863280746341353
        s = np.random.SeedSequence(entropy)
        rng = np.random.default_rng(s)
        for it in range(10):
            n = rng.poisson(20) + 1
            substring_length = rng.integers(3) + 1
            accum = rng.choice([True, False])
            lmbda = rng.uniform(0, 2)
            tgt_len_x = rng.poisson(10) + 1
            tgt_len_y = rng.poisson(10) + 1
            x = rng.integers(n, size=tgt_len_x)
            y = rng.integers(n, size=tgt_len_y)
            with self.subTest(iteration=it, n_states=n, substring_length=substring_length, accum=accum, lmbda=lmbda,
                              x=x, y=y):
                x = list_of_vec_of_single_seq(x)
                y = list_of_vec_of_single_seq(y)
                k = StringKernel(substring_length=substring_length, lmbda=lmbda, accum=accum, normalize=False)
                k_norm = StringKernel(substring_length=substring_length, lmbda=lmbda, accum=accum, normalize=True)
                kxy = k.compute_gram(x, y).item()
                kxx = k.compute_gram(x, x).item()
                kyy = k.compute_gram(y, y).item()
                k_norm_xy = k_norm.compute_gram(x, y).item()
                self.assertAlmostEqual(k_norm_xy, kxy / np.sqrt(kxx) / np.sqrt(kyy))

    def test_accum(self):
        entropy = 82317727625021695066397019016177299011
        s = np.random.SeedSequence(entropy)
        rng = np.random.default_rng(s)
        for it in range(10):
            n = rng.poisson(20) + 1
            ...
            x = rng.integers(n, size=tgt_len_x)
            y = rng.integers(n, size=tgt_len_y)
            with self.subTest(iteration=it, n_states=n, substring_length=substring_length, lmbda=lmbda,
                              x=x, y=y):
                x = list_of_vec_of_single_seq(x)
                y = list_of_vec_of_single_seq(y)
                kxy = 0
                for i in range(1, substring_length + 1):
                    k = StringKernel(substring_length=i, lmbda=lmbda, accum=False, normalize=False)
                    kxy += k.compute_gram(x, y).item()
                k_accum = StringKernel(substring_length=substring_length, lmbda=lmbda, accum=True, normalize=False)
                k_accum_xy = k_accum.compute_gram(x, y).item()
                normalizer = min(abs(kxy), abs(k_accum_xy))
                self.assertAlmostEqual(k_accum_xy / normalizer, kxy / normalizer, places=6)

    def test_kernel_is_bilinear(self):
        entropy = 146981202503099916360360509591029028960
        s = np.random.SeedSequence(entropy)
        rng = np.random.default_rng(s)
        got_nonzero_k_sum = False
        for it in range(10):
            ...
            substring_length = rng.integers(3) + 1
            accum = rng.choice([True, False])
            normalize = rng.choice([True, False])
            lmbda = rng.uniform(0, 2)
            seqs = []
            for jt in range(10):
                tgt_len = rng.poisson(4) + 1
            ...
            ixs_y = rng.choice(range(len(seqs)), replace=False, size=num_seqs)
            with self.subTest(iteration=it, n_states=n, num_seqs=num_seqs, weights_x=weights_x, weights_y=weights_y,
                              ixs_x=ixs_x, ixs_y=ixs_y, substring_length=substring_length, accum=accum,
                              normalize=normalize, lmbda=lmbda):
                kernel = StringKernel(substring_length=substring_length, lmbda=lmbda, accum=accum, normalize=normalize)
                vector_x = FreeVector()
                for weight, ix in zip(weights_x, ixs_x):
                    vector_x += weight * seqs[ix]
                vector_y = FreeVector()
